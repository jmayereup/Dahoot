...
async function runSetup() {
  ...
  // 4. Seed default questions
  try {
    console.log("[Dahoot DB] Seeding default questions...");
    for (const q of DEFAULT_QUESTIONS) {
      await pb.collection('questions').create(q);
    }
    console.log("\x1b[32m[Dahoot DB] Default questions seeded successfully!\x1b[0m");
  } catch (err) {
    console.error("[Dahoot DB] Error seeding default questions:", err.message);
  }
